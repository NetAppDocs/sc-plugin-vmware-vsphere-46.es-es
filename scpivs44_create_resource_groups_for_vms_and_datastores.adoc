...
[role="lead"]
Un grupo de recursos es el contenedor para máquinas virtuales, almacenes de datos o máquinas virtuales vVols que desea proteger.
...
+
Un almacén de datos FlexGroup. No se admite la expansión de almacenes de datos de FlexGroup. No es posible combinar almacenes de datos de FlexGroup con máquinas virtuales o almacenes de datos tradicionales.
...
| Ámbito | Seleccione el tipo de recurso que desea proteger: * Datastores (todas las máquinas virtuales tradicionales en uno o más almacenes de datos especificados) * equipos virtuales (VM individuales tradicionales o VVol; En el campo debe navegar hasta el almacén de datos que contiene las máquinas virtuales o VVol) * Etiquetas (todas las máquinas virtuales VVol con una única etiqueta de VMware especificada; en el cuadro de lista debe introducir la etiqueta) * VM Folder (todos los equipos virtuales VVol en una carpeta especificada; en el campo emergente debe desplazarse al centro de datos en el que se encuentra la carpeta) 
